...
public class WRBRESTService extends AbstractWRBService {
	/**
	 * REST service implementation of the sayHello RMI example. Plain text
	 * formated.
	 * 
	 * @param message
	 ...
	 */
	final static String INTEGRATE = "integrate";
	final static String DIFFERENTIATE = "differentiate";

	@GET
	@Path(INTEGRATE)
	@Produces(TEXT_PLAIN)
	@Consumes(TEXT_PLAIN)
	public String integrate(@DefaultValue("Hello World") @QueryParam("msg") String message) {
		Thread t = Thread.currentThread();
		Date d = new Date();
		log.info(format("GET %s PLAIN message=%s Thread: %s Time: %s", INTEGRATE, message, t, d));

		String retValue = format(" return bullshit");
